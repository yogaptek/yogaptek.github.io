<?php
	...
	if($jumlah==0) {
			...
			while($r = mysql_fetch_array($queryshow)) {
				...
				if($h==$hari && $h2==$hari) {
					...
					if($overlap <= 0) {
						...
						echo "<script>alert('Jadwal Dosen Dosen Bentrok!');location.replace('datajadwaldosen.php');</script>";
						$tidakoverlap = 1;
						break;
					}
		...
		if($querytambah)
			...
			echo "<script>alert('Data Gagal Ditambahkan!');location.replace('datajadwaldosen.php');</script>"; */
		echo "<script>alert('Data Sudah Ada!');location.replace('datajadwaldosen.php');</script>";
	}
?>